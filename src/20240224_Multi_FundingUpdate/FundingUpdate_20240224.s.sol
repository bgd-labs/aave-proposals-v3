...
contract DeployEthereum is EthereumScript {
  function run() external broadcast {
    ...
    address payload0 = GovV3Helpers.deployDeterministic(
      type(AaveV3Ethereum_FundingUpdate_20240224).creationCode
    );
...
/**
 * @dev Deploy Polygon
 * deploy-command: make deploy-ledger contract=src/20240224_Multi_FundingUpdate/FundingUpdate_20240224.s.sol:DeployPolygon chain=polygon
 * verify-command: npx catapulta-verify -b broadcast/FundingUpdate_20240224.s.sol/137/run-latest.json
 */
contract DeployPolygon is PolygonScript {
  function run() external broadcast {
    // deploy payloads
    address payload0 = GovV3Helpers.deployDeterministic(
      type(AaveV3Polygon_FundingUpdate_20240224).creationCode
    );
...
contract CreateProposal is EthereumScript {
  function run() external {
    ...
    GovV3Helpers.createProposal(
      vm,
      payloads,
      GovV3Helpers.ipfsHashFile(vm, 'src/20240224_Multi_FundingUpdate/FundingUpdate.md')
    );
  }
}
