...
import {IRiskStewardOld} from '../interfaces/IRiskStewardOld.sol';
...
abstract contract RiskStewardCooldownReductionBaseTest is ProtocolV3TestBase {
  ...
  function _testRiskStewardConfigOld(
    address payload,
    IPool pool,
    address riskStewardAddress
  ) internal {
    IRiskStewardOld riskSteward = IRiskStewardOld(riskStewardAddress);
    IRiskStewardOld.Config memory beforeConfig = riskSteward.getRiskConfig();
    ...
    IRiskStewardOld.Config memory afterConfig = riskSteward.getRiskConfig();
