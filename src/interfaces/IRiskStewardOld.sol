...
/**
 * @title IRiskStewardOld
 * @author BGD labs
 * @notice Mirror of the pre-v3.7 RiskSteward layout, where `CollateralConfig` still
 *         carries `debtCeiling`. Used to ABI-decode `getRiskConfig()` from the
 *         currently deployed (old) stewards on-chain.
 */
