...
/**
 * @dev Test for AaveV3Base_RemoteGSMLaunchArbitrum_20260512
 * command: FOUNDRY_PROFILE=test forge test --match-path=src/20260512_Multi_RemoteGSMLaunchArbitrum/AaveV3Base_RemoteGSMLaunchArbitrum_20260512.t.sol -vv
 */
contract AaveV3Base_RemoteGSMLaunchArbitrum_20260512_Test is ProtocolV3TestBase {
  AaveV3Base_RemoteGSMLaunchArbitrum_20260512 internal proposal;

  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('base'), 47979045);
    proposal = new AaveV3Base_RemoteGSMLaunchArbitrum_20260512();
  }
  ...
  function test_defaultProposalExecution() public {
    defaultTest('AaveV3Base_RemoteGSMLaunchArbitrum_20260512', AaveV3Base.POOL, address(proposal));
  }

  function test_facilitatorBucketCapacityIncrease() public {
    IGhoToken gho = IGhoToken(GhoBase.GHO_TOKEN);
    IGhoToken.Facilitator memory preFacilitator = gho.getFacilitator(GhoBase.GHO_CCIP_TOKEN_POOL);

    executePayload(vm, address(proposal));

    IGhoToken.Facilitator memory postFacilitator = gho.getFacilitator(GhoBase.GHO_CCIP_TOKEN_POOL);
