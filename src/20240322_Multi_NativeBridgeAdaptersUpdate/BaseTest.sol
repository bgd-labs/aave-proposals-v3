...
abstract contract BaseTest is ProtocolV3TestBase {
  ...
  BaseAdaptersUpdatePayload internal arbitrumPayload;
  BaseAdaptersUpdatePayload internal basePayload;
  BaseAdaptersUpdatePayload internal gnosisPayload;
  BaseAdaptersUpdatePayload internal metisPayload;
  BaseAdaptersUpdatePayload internal scrollPayload;
  BaseAdaptersUpdatePayload internal optimismPayload;
  ...
  function setUp() public {
    ...
    ethereumPayload = new AaveV3Ethereum_NativeBridgeAdaptersUpdate_20240322();
    polygonPayload = new AaveV3Polygon_NativeBridgeAdaptersUpdate_20240322();
    arbitrumPayload = new AaveV3Arbitrum_NativeBridgeAdaptersUpdate_20240322();
    basePayload = new AaveV3Base_NativeBridgeAdaptersUpdate_20240322();
    gnosisPayload = new AaveV3Gnosis_NativeBridgeAdaptersUpdate_20240322();
    metisPayload = new AaveV3Metis_NativeBridgeAdaptersUpdate_20240322();
    optimismPayload = new AaveV3Optimism_NativeBridgeAdaptersUpdate_20240322();
    scrollPayload = new AaveV3Scroll_NativeBridgeAdaptersUpdate_20240322();
  }
  ...
  function getPayloadByChainId(uint256 chainId) public view returns (BaseAdaptersUpdatePayload) {
    if (chainId == ChainIds.MAINNET) {
      ...
      return arbitrumPayload;
    } else if (chainId == ChainIds.OPTIMISM) {
      return optimismPayload;
  ...
  function getTrustedRemoteByChainId(uint256 chainId) public pure returns (address) {
    if (chainId == ChainIds.MAINNET) {
  ...
  function test_correctAdapterNames() public {
    ...
    for (uint256 i = 0; i < receivers.length; i++) {
      string memory contractAdapterName = IBaseAdapter(receivers[i].bridgeAdapter).adapterName();
      assertEq(keccak256(abi.encode(contractAdapterName)), keccak256(abi.encode(adapterName)));
    }
  }
  ...
  function _getCurrentForwarderAdaptersByChain()
    internal
    view
    returns (ForwarderAdapters[] memory)
  {
  ...
  function _getCurrentReceiverAdaptersByChain() internal view returns (AdaptersByChain[] memory) {
