...
/**
 * @dev Test for AaveV3XLayer_UpdateUSDGPriceFeed_20260514
 * command: FOUNDRY_PROFILE=test forge test --match-path=src/20260514_Multi_UpdateUSDGPriceFeed/AaveV3XLayer_UpdateUSDGPriceFeed_20260514.t.sol -vv
 */
contract AaveV3XLayer_UpdateUSDGPriceFeed_20260514_Test is ProtocolV3TestBase {
  address internal constant USDG_PRICE_FEED = 0xe00B2732396a1f047d4A00e0165025A9cF400245;
  address internal constant USDG_USD_CHAINLINK_FEED = 0x385C6bDDE06b0E438319bF4ddBfFe51C521ABf3D;

  AaveV3XLayer_UpdateUSDGPriceFeed_20260514 internal proposal;

  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('xlayer'), 63357897);
    proposal = new AaveV3XLayer_UpdateUSDGPriceFeed_20260514();
  }

  function test_defaultProposalExecution() public {
    defaultTest('AaveV3XLayer_UpdateUSDGPriceFeed_20260514', AaveV3XLayer.POOL, address(proposal));
  }

  function test_usdgPriceFeedMigratedToCapo() public {
    address before = AaveV3XLayer.ORACLE.getSourceOfAsset(AaveV3XLayerAssets.USDG_UNDERLYING);
    assertEq(before, AaveV3XLayerAssets.USDG_ORACLE, 'USDG not on the legacy feed');

    GovV3Helpers.executePayload(vm, address(proposal));

    assertEq(
      AaveV3XLayer.ORACLE.getSourceOfAsset(AaveV3XLayerAssets.USDG_UNDERLYING),
      USDG_PRICE_FEED,
  ...
  function test_usdgOraclePriceReflectsCapo() public {
    ...
    uint256 price = AaveV3XLayer.ORACLE.getAssetPrice(AaveV3XLayerAssets.USDG_UNDERLYING);
    assertEq(
      price,
  ...
  function test_reservesUseNewPriceFeed() public {
    ...
    _assertReserveUsesFeed(AaveV3XLayerAssets.USDG_UNDERLYING);
  }
  ...
  function _assertReserveUsesFeed(address asset) internal view {
    assertTrue(
      AaveV3XLayer.POOL.getReserveData(asset).aTokenAddress != address(0),
      'asset is not a listed reserve'
    );
    IAaveOracle oracle = IAaveOracle(AaveV3XLayer.POOL.ADDRESSES_PROVIDER().getPriceOracle());
