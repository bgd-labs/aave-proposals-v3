...
import {AaveV3Avalanche_ActivateCapoRiskAgentAndExpandRatesAgentOnMoreNetworks_20260130, IRangeValidationModule, IAaveCLRobotOperator} from './AaveV3Avalanche_ActivateCapoRiskAgentAndExpandRatesAgentOnMoreNetworks_20260130.sol';
  ...
  ProtocolV3TestBase
{
  ...
  function test_agentsRegistered() public {
    uint256 oldAgentCount = IAgentHub(MiscAvalanche.AGENT_HUB).getAgentCount();
    GovV3Helpers.executePayload(vm, address(proposal));

    uint256 currentAgentCount = IAgentHub(MiscAvalanche.AGENT_HUB).getAgentCount();
    assertEq(AGENT_ID, currentAgentCount - 1);
    assertEq(currentAgentCount, oldAgentCount + 1);

    assertEq(IAgentHub(MiscAvalanche.AGENT_HUB).getUpdateType(AGENT_ID), 'RateStrategyUpdate');
  }
  ...
  function test_rangeValidationConfiguration() public {
    ...
    ).getDefaultRangeConfig(MiscAvalanche.AGENT_HUB, AGENT_ID, 'VariableRateSlope2');
  ...
  function test_validateAgentContractImmutables() public virtual {
    ...
    address agentContract = IAgentHub(MiscAvalanche.AGENT_HUB).getAgentAddress(AGENT_ID);
    assertEq(
      address(IBaseAaveAgent(agentContract).RANGE_VALIDATION_MODULE()),
      address(MiscAvalanche.RANGE_VALIDATION_MODULE)
    );
    assertEq(address(IBaseAaveAgent(agentContract).POOL()), address(AaveV3Avalanche.POOL));
    assertEq(address(IBaseAaveAgent(agentContract).AGENT_HUB()), MiscAvalanche.AGENT_HUB);
  }

  function test_riskAdminRole() public {
    address agentContract = proposal.RATES_AGENT();
    assertFalse(AaveV3Avalanche.ACL_MANAGER.isRiskAdmin(agentContract));
    GovV3Helpers.executePayload(vm, address(proposal));

    assertTrue(AaveV3Avalanche.ACL_MANAGER.isRiskAdmin(agentContract));
  }

  function test_automationRegistered() public {
    vm.expectEmit(false, true, true, true, MiscAvalanche.AAVE_CL_ROBOT_OPERATOR);
  ...
  function test_injection_slope2() public {
    ...
    for (uint256 i = 0; i < markets.length && markets[i] != address(0); i++) {
      ...
      address rateStrategyAddress = AaveV3Avalanche
      ...
      IRiskOracle(AaveV3Avalanche.EDGE_RISK_ORACLE).publishRiskParameterUpdate(
        'referenceId',
        abi.encode(newRate),
        'RateStrategyUpdate',
        markets[i],
  ...
  function _checkAndPerformAutomation() public returns (bool) {
    ...
    if (upkeepNeeded) {
      AutomationCompatibleInterface(MiscAvalanche.AGENT_HUB_AUTOMATION).performUpkeep(performData);
    }
    return upkeepNeeded;
  }
}
