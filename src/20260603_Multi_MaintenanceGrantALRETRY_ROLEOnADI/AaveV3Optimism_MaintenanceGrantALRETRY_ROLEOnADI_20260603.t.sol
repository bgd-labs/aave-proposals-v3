...
contract AaveV3Optimism_MaintenanceGrantALRETRY_ROLEOnADI_20260603_Test is
  MaintenanceGrantALRETRY_ROLEOnADITestBase
{
  function _createFork() internal override {
    vm.createSelectFork(vm.rpcUrl('optimism'), 153012974);
  }

  function _deployProposal() internal override returns (IRetryRoleProposal) {
    return
      IRetryRoleProposal(address(new AaveV3Optimism_MaintenanceGrantALRETRY_ROLEOnADI_20260603()));
  }
  ...
  function _reportName() internal pure override returns (string memory) {
    return 'AaveV3Optimism_MaintenanceGrantALRETRY_ROLEOnADI_20260603';
  }
}
