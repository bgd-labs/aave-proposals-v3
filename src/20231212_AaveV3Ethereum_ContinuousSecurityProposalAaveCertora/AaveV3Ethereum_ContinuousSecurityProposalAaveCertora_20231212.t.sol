...
contract AaveV3Ethereum_ContinuousSecurityProposalAaveCertora_20231212_Test is ProtocolV3TestBase {
  ...
  function test_defaultProposalExecution() public {
    ...
    uint256 CERTORAGHOBalanceBefore = IERC20(AaveV3EthereumAssets.GHO_UNDERLYING).balanceOf(CERTORA_TREASURY);
    uint256 CERTORAAAVEBalanceBefore = IERC20(AaveV3EthereumAssets.AAVE_UNDERLYING).balanceOf(CERTORA_TREASURY);
    ...
    uint256 ReserveAAVEBalanceBefore = IERC20(AaveV3EthereumAssets.AAVE_UNDERLYING).balanceOf(address(MiscEthereum.AAVE_ECOSYSTEM_RESERVE_CONTROLLER));
...
// scoping to avoid the "stack too deep" error
    ...
    }
    {
      ...
      ) = ICollector(address(MiscEthereum.AAVE_ECOSYSTEM_RESERVE_CONTROLLER)).getStream(nextReserveStreamIDAAVE);
    ...
    uint256 nextCERTORAGHOBalance = IERC20(AaveV3EthereumAssets.GHO_UNDERLYING).balanceOf(CERTORA_TREASURY);
    ...
    MiscEthereum.AAVE_ECOSYSTEM_RESERVE_CONTROLLER.withdrawFromStream(address(MiscEthereum.AAVE_ECOSYSTEM_RESERVE_CONTROLLER), nextReserveStreamIDAAVE, ACTUAL_STREAM_AMOUNT_AAVE);
    uint256 nextCERTORAAAVEBalance = IERC20(AaveV3EthereumAssets.AAVE_UNDERLYING).balanceOf(CERTORA_TREASURY);
