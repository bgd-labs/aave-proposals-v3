...
contract AaveV3Gnosis_RiskStewardPhase2_20240805_Test is ProtocolV3TestBase {
  ...
  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('gnosis'), 35324260);
  ...
  function test_permissions() public {
    assertEq(AaveV3Gnosis.ACL_MANAGER.isRiskAdmin(AaveV3Gnosis.CAPS_PLUS_RISK_STEWARD), true);
    executePayload(vm, address(proposal));

    assertEq(AaveV3Gnosis.ACL_MANAGER.isRiskAdmin(AaveV3Gnosis.CAPS_PLUS_RISK_STEWARD), false);
    assertEq(AaveV3Gnosis.ACL_MANAGER.isRiskAdmin(proposal.NEW_RISK_STEWARD()), true);
  }
}
