...
contract AaveV3Polygon_RiskStewardPhase2_20240805_Test is ProtocolV3TestBase {
  ...
  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('polygon'), 60210763);
  ...
  function test_permissions() public {
    assertEq(AaveV3Polygon.ACL_MANAGER.isRiskAdmin(AaveV3Polygon.CAPS_PLUS_RISK_STEWARD), true);
    executePayload(vm, address(proposal));

    assertEq(AaveV3Polygon.ACL_MANAGER.isRiskAdmin(AaveV3Polygon.CAPS_PLUS_RISK_STEWARD), false);
    assertEq(AaveV3Polygon.ACL_MANAGER.isRiskAdmin(proposal.NEW_RISK_STEWARD()), true);
  }
}
