...
import {Helpers} from 'aave-helpers/src/dependencies/v4/Helpers.sol';
...
abstract contract Scenarios is Helpers {
  ...
  ///      Chain-agnostic: derives the access manager from the spoke's `authority()`,
  ///      mocks it to bypass auth, then deploys a fresh SpokeConfigurator wired to that
  ///      access manager and calls `updateCollateralFactor` (mirrors
  ///      `_deployMockedHubConfigurator`). Avoids any hardcoded per-chain configurator.
  function _updateCollateralFactor(
    ...
    address accessManager = spoke.authority();
    vm.mockCall(
      accessManager,
      abi.encodeWithSelector(bytes4(keccak256('canCall(address,address,bytes4)'))),
      abi.encode(true, uint32(0))
    );
    ISpokeConfigurator spokeConfigurator = new SpokeConfigurator(accessManager);
