  ...
  IProposalGenericExecutor
{
  ...
  function execute() external {
    address[] memory reserves = AaveV3Linea.POOL.getReservesList();
    uint256 reservesLen = reserves.length;

    for (uint256 i = 0; i < reservesLen; i++) {
      address reserve = reserves[i];
      address aToken = AaveV3Linea.POOL.getReserveAToken(reserve);
      DataTypes.ReserveConfigurationMap memory configuration = AaveV3Linea.POOL.getConfiguration(
        reserve
      );
      ...
      uint256 balanceDustBin = IERC20(aToken).balanceOf(AaveV3Linea.DUST_BIN);

      if (balanceDustBin < tokenAmount) {
        uint256 balanceCollector = IERC20(aToken).balanceOf(address(AaveV3Linea.COLLECTOR));
        uint256 toSend = tokenAmount - balanceDustBin;
        AaveV3Linea.COLLECTOR.transfer(
          IERC20(aToken),
