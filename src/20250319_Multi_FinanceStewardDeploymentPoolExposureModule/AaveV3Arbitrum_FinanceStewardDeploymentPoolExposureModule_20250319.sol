  ...
  IProposalGenericExecutor
{
  ...
  function execute() external {
    ...
    for (uint256 i = 0; i < reservesLen; i++) {
      ...
      address aToken = AaveV3Arbitrum.POOL.getReserveAToken(reserve);
      DataTypes.ReserveConfigurationMap memory configuration = AaveV3Arbitrum.POOL.getConfiguration(
        reserve
      );
      ...
      uint256 balanceDustBin = IERC20(aToken).balanceOf(AaveV3Arbitrum.DUST_BIN);

      if (balanceDustBin < tokenAmount) {
        uint256 balanceCollector = IERC20(aToken).balanceOf(address(AaveV3Arbitrum.COLLECTOR));
