 ...
 */
contract AaveV3Linea_AddMetaMaskMUSDToAaveV3EthereumLinea_20251120 is AaveV3PayloadLinea {
  ...
  function _supplyAndConfigureLMAdmin(address asset, uint256 seedAmount, address lmAdmin) internal {
    ...
    AaveV3Linea.POOL.supply(asset, seedAmount, address(AaveV3Linea.DUST_BIN), 0);

    if (lmAdmin != address(0)) {
      address aToken = AaveV3Linea.POOL.getReserveAToken(asset);
      IEmissionManager(AaveV3Linea.EMISSION_MANAGER).setEmissionAdmin(asset, lmAdmin);
      IEmissionManager(AaveV3Linea.EMISSION_MANAGER).setEmissionAdmin(aToken, lmAdmin);
    }
  }
}
