...
/// Test asserting every wiring the payload declares for `targetTestRef` landed on the
/// AccessManager. Matching on the target rather than on a config position, as the getter
/// also carries the wiring of every other feature in the payload.
export function rolesWiredTest(key: string, targetTestRef: string, referenceExpr: string): string {
  return `function test_rolesWired_${key}() public {
        IConfigEngine.TargetFunctionRoleUpdate[] memory items = proposal.accessManagerTargetFunctionRoleUpdates();
        GovV3Helpers.executePayload(vm, address(proposal));
        uint256 matched;
        for (uint256 i; i < items.length; ++i) {
          if (items[i].target != ${targetTestRef}) continue;
...
export const accessManagerTargetFunctionRoleUpdate: FeatureModule<V4TargetFunctionRoleUpdate[]> = {
  ...
  build({market, cfg}) {
    ...
    // one test per target, as the same target named twice yields a single test
    ...
    const testFns = [...byTarget.values()].map((c) =>
      rolesWiredTest(targetIdentifier(c), targetTestCodegen(c), 'address(0)'),
    );
