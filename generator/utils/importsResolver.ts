...
export function prefixWithImports(code: string) {
  ...
  if (findMatch(code, 'EngineFlags')) {
    imports += `import {EngineFlags} from 'aave-v3-periphery/v3-config-engine/EngineFlags.sol';\n`;
  }
  // v3 config engine imports
  if (findMatch(code, 'IAaveV3ConfigEngine')) {
    imports += `import {IAaveV3ConfigEngine} from 'aave-v3-periphery/v3-config-engine/IAaveV3ConfigEngine.sol';\n`;
  }
